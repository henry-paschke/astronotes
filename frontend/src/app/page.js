...
const CONSTELLATIONS = [
  { nodes: [[95,70],[130,105],[168,88],[200,120],[165,155],[130,105]], color: "#c4a35a" },
  { nodes: [[1100,60],[1140,95],[1175,75],[1210,110],[1248,82]], color: "#4a7fcb" },
  { nodes: [[1280,320],[1310,355],[1345,335],[1380,370],[1345,405],[1310,355]], color: "#c4a35a" },
  { nodes: [[60,560],[95,530],[130,555],[165,520],[200,548]], color: "#4a7fcb" },
  { nodes: [[1050,710],[1090,680],[1130,700],[1165,670],[1200,695],[1165,730]], color: "#c4a35a" },
];
...
function AstrolabeSVG() {
  ...
  const labels = Array.from({ length: 12 }, (_, i) => {
    ...
    return {
      x: cx + 185 * Math.cos(rad), y: cy + 185 * Math.sin(rad),
      text: deg === 0 ? "N" : deg === 90 ? "E" : deg === 180 ? "S" : deg === 270 ? "W" : String(deg),
      isBearing,
    };
  });
  ...
  const reteNotches = Array.from({ length: 24 }, (_, i) => {
    ...
    return {
      ...
      x2: cx + 138 * Math.cos(rad), y2: cy + 138 * Math.sin(rad),
    };
  });
  ...
  return (
    <svg viewBox="0 0 500 500" className={styles.astrolabeSvg} aria-hidden="true">
      <defs>
        <filter id="roughen">
      ...
      <circle cx={cx} cy={cy} r={228} fill="none" stroke="#c4a35a" strokeWidth="1.5" filter="url(#roughen)" />
      <circle cx={cx} cy={cy} r={220} fill="none" stroke="#c4a35a" strokeWidth="0.8" opacity="0.6" filter="url(#roughen)" />
      ...
      <g style={{ transformOrigin: `${cx}px ${cy}px`, animation: "rotateSlow 16s linear infinite" }}>
        <circle cx={cx} cy={cy} r={170} fill="none" stroke="#c4a35a" strokeWidth="0.6" opacity="0.4" strokeDasharray="3 5" />
        {compassStars.map((s, i) => (
          <g key={i} transform={`translate(${s.x}, ${s.y})`}>
            {[0, 72, 144, 216, 288].map((a, j) => {
              ...
              return (
                <line key={j}
                  x1={r1 * Math.cos(a1)} y1={r1 * Math.sin(a1)}
                  x2={r2 * Math.cos(a2)} y2={r2 * Math.sin(a2)}
                  stroke="#e8c878" strokeWidth="0.8" opacity="0.85"
                />
              );
            })}
            <circle r={s.size * 0.3} fill="#e8c878" opacity="0.9" filter="url(#glow)" />
          </g>
        ))}
      </g>

      {/* ── LAYER 2: Mid band + rete notches — 10s CCW ── */}
      <g style={{ transformOrigin: `${cx}px ${cy}px`, animation: "rotateReverse 10s linear infinite" }}>
        <circle cx={cx} cy={cy} r={148} fill="none" stroke="#c4a35a" strokeWidth="1" opacity="0.55" />
        {reteNotches.map((n, i) => (
          <line key={i} x1={n.x1} y1={n.y1} x2={n.x2} y2={n.y2}
            stroke="#c4a35a" strokeWidth="0.8" opacity="0.5" />
        ))}
      </g>

      {/* ── LAYER 3: Alidade cross — 24s CCW ── */}
      <g style={{ transformOrigin: `${cx}px ${cy}px`, animation: "rotateReverse 24s linear infinite" }}>
        <line x1={cx} y1={cy - 218} x2={cx} y2={cy + 218} stroke="#c4a35a" strokeWidth="0.6" opacity="0.3" />
        <line x1={cx - 218} y1={cy} x2={cx + 218} y2={cy} stroke="#c4a35a" strokeWidth="0.6" opacity="0.3" />
        <line x1={cx - 154} y1={cy - 154} x2={cx + 154} y2={cy + 154} stroke="#c4a35a" strokeWidth="0.5" opacity="0.18" />
        <line x1={cx + 154} y1={cy - 154} x2={cx - 154} y2={cy + 154} stroke="#c4a35a" strokeWidth="0.5" opacity="0.18" />
      </g>

      {/* ── LAYER 4: Inner blue rings — 6s CW ── */}
      <g style={{ transformOrigin: `${cx}px ${cy}px`, animation: "rotateSlow 6s linear infinite" }}>
        <circle cx={cx} cy={cy} r={130} fill="none" stroke="#4a7fcb" strokeWidth="0.7" opacity="0.3" strokeDasharray="1 14" />
        <circle cx={cx} cy={cy} r={110} fill="none" stroke="#4a7fcb" strokeWidth="0.8" opacity="0.45" strokeDasharray="2 8" />
      </g>

      {/* ── STATIC: Center pin ── */}
      <circle cx={cx} cy={cy} r={18} fill="none" stroke="#c4a35a" strokeWidth="1" opacity="0.6" filter="url(#roughen)" />
      <circle cx={cx} cy={cy} r={6} fill="#c4a35a" opacity="0.7" filter="url(#glow)" />
...
function MicIcon() {
  return (
    <svg viewBox="0 0 64 64" className={styles.featureIcon} aria-hidden="true">
      <ellipse cx="32" cy="32" rx="28" ry="10" fill="none" stroke="#c4a35a" strokeWidth="0.7" opacity="0.35" />
      <ellipse cx="32" cy="32" rx="10" ry="28" fill="none" stroke="#4a7fcb" strokeWidth="0.7" opacity="0.3" />
      <rect x="26" y="12" width="12" height="22" rx="6" fill="none" stroke="#c4a35a" strokeWidth="1.5" />
      <path d="M22 33 Q22 46 32 46 Q42 46 42 33" fill="none" stroke="#c4a35a" strokeWidth="1.5" />
      <line x1="32" y1="46" x2="32" y2="54" stroke="#c4a35a" strokeWidth="1.5" />
      <line x1="24" y1="54" x2="40" y2="54" stroke="#c4a35a" strokeWidth="1.5" />
...
function ConstellationIcon() {
  const nodes = [[32,12],[52,28],[44,50],[20,50],[12,28]];
  const edges = [[0,1],[1,2],[2,3],[3,4],[4,0],[0,2],[1,3]];
  return (
    <svg viewBox="0 0 64 64" className={styles.featureIcon} aria-hidden="true">
      {edges.map(([a,b], i) => (
        <line key={i} x1={nodes[a][0]} y1={nodes[a][1]} x2={nodes[b][0]} y2={nodes[b][1]}
          stroke="#4a7fcb" strokeWidth="0.8" opacity="0.45" />
      ))}
      {nodes.map(([x,y], i) => (
        <circle key={i} cx={x} cy={y} r={i===0?4:2.5} fill={i===0?"#e8c878":"#c4a35a"} opacity={i===0?1:0.8} />
      ))}
    </svg>
  );
}
...
function CompassIcon() {
  return (
    <svg viewBox="0 0 64 64" className={styles.featureIcon} aria-hidden="true">
      <circle cx="32" cy="32" r="26" fill="none" stroke="#c4a35a" strokeWidth="1" opacity="0.5" />
      <circle cx="32" cy="32" r="20" fill="none" stroke="#c4a35a" strokeWidth="0.6" opacity="0.3" strokeDasharray="2 4" />
      ...
      {[0,90,180,270].map((deg,i) => {
        const rad = ((deg-90)*Math.PI)/180;
        return <line key={i} x1={32+22*Math.cos(rad)} y1={32+22*Math.sin(rad)} x2={32+26*Math.cos(rad)} y2={32+26*Math.sin(rad)} stroke="#c4a35a" strokeWidth="1.5" />;
      })}
    ...
    <div className={styles.page}>
      ...
      <nav className={styles.nav}>
        <a href="/" className={styles.navLogo}>
          <svg viewBox="0 0 28 28" className={styles.navLogoMark} aria-hidden="true">
            {/* Outer ring — dashed, rotates CW. C ≈ 75.4 → gap of ~15px */}
            ...
            />
            {/* Inner ring — dashed, rotates CCW. C ≈ 44 → gap of ~8px */}
        ...
        <ul className={styles.navLinks}>
          ...
          <li><a href="#" className={styles.navCta}>Begin Voyage</a></li>
        </ul>
      </nav>
      ...
      <section className={styles.hero}>
        ...
        >
          ...
          {CONSTELLATIONS.map((group, gi) =>
            group.nodes.slice(0, -1).map((node, ni) => (
              <line key={`cl-${gi}-${ni}`}
                x1={node[0]} y1={node[1]}
                x2={group.nodes[ni+1][0]} y2={group.nodes[ni+1][1]}
                stroke={group.color} strokeWidth="0.6" opacity="0.35"
              />
            ))
          )}
          {CONSTELLATIONS.map((group, gi) =>
            group.nodes.map(([x,y], ni) => (
              <circle key={`cn-${gi}-${ni}`} cx={x} cy={y}
                r={ni===0?2:1.2} fill={group.color} opacity="0.7"
                filter="url(#starGlow)"
              />
            ))
          )}
        ...
        <div className={styles.heroInner}>
          <div className={styles.heroText}>
            ...
            <p className={styles.heroTagline}>
              Chart the cosmos of knowledge. Record lectures, forge constellation
              mind&#8209;maps, and wield celestial study tools &mdash; all in one instrument.
            </p>
      ...
      <section id="features" className={styles.features}>
        <div className={styles.sectionHeader}>
          ...
          <p className={styles.sectionSub}>
            Three celestial tools &mdash; one unified observatory for your studies
          </p>
        </div>
        ...
        <div className={styles.featureCards}>
          <article className={styles.featureCard}>
            ...
            <div className={styles.featureIconWrap}><MicIcon /></div>
            <h3 className={styles.featureTitle}>Stellar Recorder</h3>
            <p className={styles.featureDesc}>
              Capture lectures with precision. Timestamps anchor your notes to the exact
              moment of discovery, so no insight drifts past the horizon.
            </p>
            <a href="#" className={styles.featureLink}>Explore &rarr;</a>
          </article>

          <article className={`${styles.featureCard} ${styles.featureCardFeatured}`}>
            <div className={styles.featureCardGlow} />
            <div className={styles.featureIconWrap}><ConstellationIcon /></div>
            <h3 className={styles.featureTitle}>Constellation Maps</h3>
            <p className={styles.featureDesc}>
              Weave ideas into mind maps that mirror the night sky. Draw connections
              the way navigators once charted stars between worlds.
            </p>
            <a href="#" className={styles.featureLink}>Explore &rarr;</a>
          </article>
          ...
          <article className={styles.featureCard}>
            <div className={styles.featureCardGlow} />
            <div className={styles.featureIconWrap}><CompassIcon /></div>
      ...
      <section className={styles.ctaBanner}>
        <div className={styles.ctaBannerInner}>
          ...
          <a href="#" className={styles.ctaPrimary}>Open the Observatory</a>
        </div>
      </section>
        ...
        <div className={styles.footerInner}>
          <div className={styles.footerBrand}>
            <svg viewBox="0 0 20 20" width="18" height="18" aria-hidden="true">
              ...
              <line x1="10" y1="1.5" x2="10" y2="18.5" stroke="#c4a35a" strokeWidth="0.5" opacity="0.3" />
              <line x1="1.5" y1="10" x2="18.5" y2="10" stroke="#c4a35a" strokeWidth="0.5" opacity="0.3" />
            </svg>
          ...
          <p className={styles.footerCopy}>&copy; 2026 AstroNotes &mdash; Chart your course</p>
        </div>
      </footer>
    </div>
  );
